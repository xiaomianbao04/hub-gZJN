...
print(f"DASHSCOPE_API_KEY 读取结果: {dashscope_key[:10] if dashscope_key else 'None'}...")
...
def _parse_step(text: str) -> dict:
    ...
    try:
        action_input = json.loads(input_m.group(1)) if input_m else {}#把入参转为字典是为了后面工具调用解包使用
...
def run(question: str, max_steps: int = 10, verbose: bool = True, history: list = None) -> Generator[dict, None, None]:
    ...
    参数:
      history: 历史对话记录，格式为 [{"role": "user", "content": "...", "answer": "..."}, ...]
               每个元素包含用户问题和对应的最终答案，用于多轮对话记忆
    """
